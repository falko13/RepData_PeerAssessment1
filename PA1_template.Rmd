...
**median** = *`r round(median(df_st1$sumByDate,na.rm=T),2)`*    
**mean** = *`r round(mean(df_st1$sumByDate,na.rm=T),2)`*
...
**max steps 5-interval** = *`r df_st2[df_st2[,2] == max(df_st2[,2]),"interval"][[1]]`*
...
For now sample includes *`r nrow(df[!complete.cases(df),])`* **missing values**.
...
**Median** = *`r round(median(df_st3$sumByDate,na.rm=T),2)`*  
**Mean** = *`r round(mean(df_st3$sumByDate,na.rm=T),2)`*  
Both values remains almost the same as before imputing.  
median(NA free)-median(NA) = *`r round(median(df_st3$sumByDate,na.rm=T),2)-round(median(df_st1$sumByDate,na.rm=T),2)`*  
mean(NA free)-mean(NA) = *`r round(mean(df_st3$sumByDate,na.rm=T),2)-round(mean(df_st1$sumByDate,na.rm=T),2)`*
